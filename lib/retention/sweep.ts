...
export async function runRetentionSweep(companyId: string, limit = 100) {
  const safeLimit = Math.max(1, Math.min(Math.floor(limit), 500));
  return tx(async () => {
    const { retentionMonths } = await getRetentionPolicy(companyId);
    const cutoff = retentionCutoff(retentionMonths);
    const candidates = await query<{ id: string }>(
      `SELECT application.id
       FROM applications application JOIN jobs job ON job.id = application.job_id
       WHERE job.company_id = ? AND application.submitted_at < ? AND NOT application.legal_hold
    ...
    const legalHold = await queryOne<{ count: number }>(
      `SELECT COUNT(*) AS count FROM applications application JOIN jobs job ON job.id = application.job_id
       WHERE job.company_id = ? AND application.legal_hold`,
      [companyId],
    );
    const startedAt = new Date().toISOString();
    let pseudonymizedCount = 0;
    for (const candidate of candidates) if (await pseudonymizeApplication(candidate.id)) pseudonymizedCount += 1;
