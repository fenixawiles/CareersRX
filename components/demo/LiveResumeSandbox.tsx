...
export function LiveResumeSandbox({
  initialSnapshot,
  apiBase = "/api/demo/live-resume",
  profileHref = "/demo/profile",
  setupHref = "/demo/live-resume/signup",
  setupLabel,
  mode = "demo",
}: LiveResumeSandboxProps) {
  ...
  const [status, setStatus] = useState("Blank sandbox ready");
  ...
  async function saveDraft(
    sectionId?: SandboxResumeSection["id"],
    nextStatus = "Draft saved to SQLite",
    sectionsToSave = sections,
  ) {
    setStatus(sectionId ? "Saving section to SQLite…" : "Saving changes to SQLite…");
  ...
  async function reset() {
    setStatus("Resetting blank sandbox…");
    const response = await fetch(apiBase, { method: "DELETE" });
    await refreshFromResponse(response, "Blank sandbox reset");
  }
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-border bg-surface p-3 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-[260px] flex-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {mode === "demo" ? <Badge tone="primary">No login</Badge> : <Badge tone="success">Logged in</Badge>}
            ...
            <div className="relative">
              ...
              {actionMenuOpen ? (
                <div className="absolute right-0 top-[calc(100%+0.5rem)] z-20 w-64 rounded-2xl border border-border bg-surface p-2 text-sm shadow-xl">
                  ...
                  {mode === "demo" ? (
                    <Button type="button" variant="ghost" size="sm" onClick={reset} disabled={isPending} className="w-full justify-start">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-3 border-t border-border pt-2 text-xs text-muted">
          ...
          <span className="inline-flex items-center gap-1">
            <Database size={14} /> Local SQLite persistence enabled
          </span>
        </div>
      </div>
      ...
      {!hasProfile ? (
        <div className="rounded-2xl border border-warning/30 bg-[#f6ecd8]/50 p-4 text-sm text-foreground">
          <strong>{mode === "demo" ? "Create a sandbox profile first" : "Finish your profile first"}</strong>{" "}
      ...
      <div className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <main className="space-y-4 rounded-2xl border border-border bg-surface p-4">
          ...
          {sections.map((section) => (
            <section id={`sandbox-${section.id}`} key={section.id} className="rounded-2xl border border-border bg-background p-4">
